...
const getTargets2Scan = async (prisma: PrismaClient) => {
  ...
  ).getTime()} OR "lastScan" IS NULL) OR MOD(number + ${currentMinute}, ${scanIntervalMinutes}) = 0) AND "queued" = false AND "errorCount" < 5 AND exists(SELECT 1 from target_collections tc WHERE tc.uri = targets.uri)`)) as Array<Target>;
